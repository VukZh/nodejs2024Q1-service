...
export class ArtistController {
  ...
  async addArtist(@Body() body: ArtistDto ): Promise<ArtistDto> {
    console.log("Post album ", body);
    return this.artistService.createArtist(body);
  };

  @Put("/:id")
  async changeArtist(@Body() body: ArtistDto , @Param("id", ParseUUIDPipe) id: string): Promise<ArtistDto> {
    return this.artistService.updateArtist(body, id);
  };

  @Delete("/:id")
  async deleteArtist(@Param("id", ParseUUIDPipe) id: string): Promise<HttpException> {
